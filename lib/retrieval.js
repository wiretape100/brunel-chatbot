...
export function describeRetrievalPlan(plan) {
  ...
  return [
    ...
    `Requested concepts: ${plan.concepts.map(conceptLabel).join(", ")}.`,
    "The search covered the full original query, the combined concept phrase, each concept separately, and synonym expansions.",
    "When answering, count the requested concepts correctly. Use wording like all three topics, two of the three topics, or one of the three topics depending on the confirmed topic-source map.",
    ...
    "Only list a source title as a confirmed source if it appears in the confirmed topic-source map with a URL."
  ].join("\n");
}
...
export function sourceMatchesConcept(item, concept) {
  return itemMatchesConcept(item, concept);
}
