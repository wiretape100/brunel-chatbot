...
function parseWorkbook(workbook, workbookPath, mapping) {
  ...
  const rawSheets = chooseRawSheets(workbook.SheetNames);
  const facts = parseRawFacts(workbook, rawSheets, workbookPath, mapping, metadata);

  return {
    workbookName: path.basename(workbookPath),
    analysisSheet: sheetName,
    rawSheetNames: rawSheets,
    headers,
    rows,
    facts,
    metadata,
...
function chooseRawSheets(sheetNames) {
  return sheetNames.filter((name) => name.toLowerCase().includes("raw"));
}
...
function parseRawFacts(workbook, rawSheets, workbookPath, mapping, metadata) {
  ...
  for (const sheetName of rawSheets) {
    ...
    const sheetFacts = parseWideYearFacts(rawRows, sheetName, workbookPath, mapping, metadata);
...
function createDatasetSummary(mapping, parsed) {
  ...
  return normalizeText([
    ...
    parsed.rawSheetNames.length ? `Raw sheets: ${parsed.rawSheetNames.join(", ")}` : null,
