...
export async function buildDataHubCatalogueAnswer({ message, history = [] }) {
  ...
  return {
    answer: renderCatalogueAnswerWithLinks({
      batch,
...
function renderCatalogueAnswerWithLinks({ batch, totalCount, topic, kind, showAllRequested, remainingCount }) {
  ...
  const intro = showAllRequested
    ...
    : kind === "more"
      ...
      : `Here are some ${topicText}Data Hub insights I found for the Greater West of England. This is not the full list. You can ask me to show more or filter by topic.`;
  ...
  if (remainingCount > 0) {
    lines.push("Say \"show more\" to continue, or ask for a topic such as housing, skills, productivity, health, transport or environment.");
  } else if (kind === "more") {
    ...
    lines.push("Would you like me to filter by a topic such as housing, skills, productivity, health, transport or environment?");
  }
...
function selectCatalogueBatch(items, { count, topic }) {
  ...
  const queues = GROUP_ORDER.map((groupName) => [...(grouped.get(groupName) || [])]);
  const batch = [];
...
function groupForItem(item) {
  ...
  if (phraseInText(text, "housing") || phraseInText(text, "house prices") || phraseInText(text, "affordability") || phraseInText(text, "dwellings") || phraseInText(text, "housebuilding")) return "Housing";
...
function hasPreviousCatalogueAnswer(history) {
  return (history || []).some((item) =>
    item?.role === "assistant" &&
    /Data Hub insights I found|show more|narrow by topic|filter by topic/i.test(String(item.content || ""))
  );
}
