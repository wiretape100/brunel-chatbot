...
export function inferSourceType(source) {
  ...
  if (path === "/consultancy" || path.startsWith("/services/") || tags.includes("consultancy") || tags.includes("service")) {
    return "servicePage";
  }
  ...
  if (path.startsWith("/reports/") || tags.includes("report")) return "reportPage";
  if (path.startsWith("/projects/") || tags.includes("project")) return "projectPage";
...
export function isGenericPageSource(source) {
  ...
  return ["homePage", "genericPage", "servicePage", "otherPage"].includes(type);
}
