...
export async function buildStatisticalAnswer({ supabase, message, contextMessage = message }) {
  const lookupMessage = contextMessage || message;
  const employmentCountAnswer = await buildEmploymentCountAnswer({ supabase, message, contextMessage: lookupMessage });
  if (employmentCountAnswer) return employmentCountAnswer;

  const employmentOverallRateAnswer = await buildEmploymentOverallRateAnswer({ supabase, message, contextMessage: lookupMessage });
  if (employmentOverallRateAnswer) return employmentOverallRateAnswer;

  const employmentRateAnswer = await buildEmploymentRateAnswer({ supabase, message, contextMessage: lookupMessage });
  if (employmentRateAnswer) return employmentRateAnswer;

  const separateNeetActivityAnswer = await buildSeparateNeetActivityAnswer({ supabase, message, contextMessage: lookupMessage });
...
async function buildEmploymentRateAnswer({ supabase, message, contextMessage = message }) {
  const lookupMessage = contextMessage || message;
  ...
  try {
    ...
    const directGeographies = matchRequestedEmploymentGeographies(rows, message);
    const geographies = directGeographies.length
      ? directGeographies
      : matchRequestedEmploymentGeographies(rows, lookupMessage);
...
async function buildEmploymentCountAnswer({ supabase, message, contextMessage = message }) {
  const lookupMessage = contextMessage || message;
  ...
  try {
    ...
    return renderEmploymentCountsNotFoundAnswer(targetGeographies);
  } catch {
    return null;
  }
}

async function buildEmploymentOverallRateAnswer({ supabase, message, contextMessage = message }) {
  const lookupMessage = contextMessage || message;
...
function isEmploymentCountQuestion(message, lookupMessage) {
  return hasCountDetailIntent(message) &&
    ...
    !hasFuzzyKeyword(lookupMessage, ["neet", "neets"]);
}
...
function renderEmploymentCountsNotFoundAnswer(geographies) {
  ...
  return {
    answer: [
      `I found employment rates${geographyText} in the Brunel Centre source, but I could not find employment counts in the article or the linked Data Hub dataset for that source.`,
      "",
      "The available figures I found are rates, not counts.",
...
function matchRequestedEmploymentGeographies(rows, message) {
  const normalizedMessage = normalizeForMatch(message);
...
async function buildSeparateNeetActivityAnswer({ supabase, message, contextMessage = message }) {
  const lookupMessage = contextMessage || message;
  ...
  try {
    ...
    const neetFacts = await fetchCalculationFacts({ supabase, geographies, requestedYear, calculation: neetCalculation });
    const activityFacts = await fetchCalculationFacts({ supabase, geographies, requestedYear, calculation: activityCalculation });
