...
const CALCULATION_INTENT = /\b(calculate|combined|combine|weighted|population[-\s]?weighted|aggregate|together|rate|percentage)\b/i;

export async function buildStatisticalAnswer({ supabase, message }) {
  const calculation = chooseRateCalculation(message);
  if (!calculation || !CALCULATION_INTENT.test(message)) return null;
  ...
  try {
    ...
    if (geographies.length < 2 && /\b(combined|combine|weighted|population[-\s]?weighted|aggregate|together)\b/i.test(message)) {
      return {
        answer: [
    ...
    return renderRateAnswer({ calculation, group, geographies, requestedYear });
  } catch {
    return null;
  }
}
...
function renderRateAnswer({ calculation, group, geographies, requestedYear }) {
