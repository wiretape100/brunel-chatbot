...
export async function buildStatisticalAnswer({ supabase, message }) {
  ...
  if (!calculation || !hasRateLookupIntent(message)) return null;
  ...
  try {
    ...
    return renderIndividualRateAnswer({ calculation, group, geographies, requestedYear });
  } catch {
    return null;
  }
}
...
async function fetchCalculationFacts({ supabase, geographies, requestedYear, calculation }) {
  let query = supabase
    ...
    .in("measure", [calculation.numeratorMeasure, calculation.denominatorMeasure])
    .limit(2000);
...
function renderIndividualRateAnswer({ calculation, group, geographies, requestedYear }) {
  const yearNote = requestedYear
    ? `For ${group.year}:`
  ...
  const lines = geographies.map((geography) => {
    ...
    const numerator = facts[calculation.numeratorMeasure];
    const denominator = facts[calculation.denominatorMeasure];
    const rate = denominator.value > 0 ? (numerator.value / denominator.value) * 100 : null;
  ...
  return {
    answer: [
      ...
      geographies.length > 1
        ? `Because the cohort counts are available, I can also calculate a population-weighted combined ${calculation.label} for these areas.`
        : null
    ].filter(Boolean).join("\n"),
    sources: [
      {
...
function renderSimpleAverageRejectedAnswer({ calculation, group, geographies, requestedYear }) {
  ...
  return {
    answer: [
      `I would not use a simple arithmetic average for this ${calculation.label} as an overall figure.`,
      "",
      "For rates/proportions with different cohort sizes, a simple average treats each area equally rather than each person/cohort member equally. That can be misleading if the question is asking for the overall rate across the selected areas.",
      "",
      "Using the available cohort counts, the appropriate combined calculation is:",
      "",
      weighted.answer
    ].join("\n"),
