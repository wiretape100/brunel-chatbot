...
const RATE_LOOKUP_INTENT = /\b(rate|percentage|percent|proportion)\b/i;
const COMBINED_RATE_INTENT = /\b(combined|combine|weighted|population[-\s]?weighted|aggregate|together)\b/i;

export async function buildStatisticalAnswer({ supabase, message }) {
  const calculation = chooseRateCalculation(message);
  if (!calculation || !RATE_LOOKUP_INTENT.test(message)) return null;

  try {
    const requestedYear = extractYear(message);
    const geographies = await matchRequestedGeographies(supabase, message);
    const wantsCombinedRate = COMBINED_RATE_INTENT.test(message);
...
async function matchRequestedGeographies(supabase, message) {
  ...
  for (const geography of geographies) {
    ...
    const matched = aliases.some((alias) => alias.length >= 3 && normalizedMessage.includes(alias));
    if (!matched) continue;
...
function renderIndividualRateAnswer({ calculation, group, geographies, requestedYear }) {
  ...
  return {
    answer: [
      ...
      "These are separate area rates. I have not combined them."
    ].join("\n"),
    sources: [
      {
        title: group.post_title,
